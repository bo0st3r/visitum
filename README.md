...
- **City Population**: I used the city proper population, but wished to use the metropolitan area population as it's likely a better proxy for the number of visitors. But because I preferred a reliable and simple solution, I went with the city proper population using the `geocoder` library and the `geonames` provider. For certain cities (like Vatican City) I hard coded rules to get more relevant data. For others cities, or when multiple cities are listed, I used the biggest city proper population of the list.
...
│
├── data/         # Output directory for generated data (DB, model, CSVs)
...
│   └── scripts/         # Utility scripts for running pipeline stages #TODO: move to root?
│       ├── __init__.py
...
- Data is stored in a Docker volume named `visitum_data_volume` which persists across container restarts.
- This volume is mapped to the `/app/data` directory inside the Docker container.
- When the application starts for the first time (or if the volume is empty), the `entrypoint.sh` script will execute the data extraction, database loading, and model training scripts, generating `visitum.db` and `trained_regression_model.joblib` in the data volume.
...
3.  To remove the stopped containers (and free up resources), you can optionally run:
    ...
    docker-compose down
    ```
    This command does not remove the Docker volume or the Docker image itself. If you want to remove the volume as well, use:
    ```bash
...
- **Some research**:
  - [Time series forecasting for museum visitors repository full of interesting ideas](https://github.com/Di40/Time-Series-Forecasting-Museum-Visitors?tab=readme-ov-file).
